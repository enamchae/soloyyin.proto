...
export default class Medi {
	...
	constructor(media) {
		...
		this.listeners = [
			Listener.for(this.media, "play", event => {
			...
			Listener.for(this.media, "playing", event => {
			...
			Listener.for(this.media, "pause", event => {
			...
			Listener.for(this.media, "waiting", event => {
			...
			Listener.for(this.media, "seeking", event => {
				const firedWhilePlaying = this.mediaState === MediaState.PLAYBACK_STARTED;
				this.mediaState = MediaState.PLAYBACK_STOPPED;
			...
			Listener.for(this.media, "loadstart", event => {
				const firedWhilePlaying = this.mediaState === MediaState.PLAYBACK_STARTED;
				this.mediaState = MediaState.PAUSED;
			...
			Listener.for(this.media, "ratechange", event => {
				dispatchEvent(this, Medi.RATECHANGE);
			}),
	
			Listener.for(this.media, "loadedmetadata", () => {
				dispatchEvent(this, Medi.LOAD_METADATA_END);
			}),
	
			Listener.for(this.media, "loadeddata", () => {
	...
	/** Stop listening to media events on the attached media. */
	deafen() {
	...
	on(eventType, handler, options) {
		return Listener.for(this.eventTarget, eventType, handler, options);
	}
...
class Listener {
	...
	static for(target, type, handler, options) {
		target.addEventListener(type, handler, options);
		return new Listener(target, type, handler, options);
	}
